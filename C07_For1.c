...
int main(){
    ...
    *///-------------------------------------
    ...
    // -------------------------------------------------
    start = 3, end=10;    int cnt = 0;
    for(int i=start; i<end-cnt; i++){
        if(i%2==0) continue;
        cnt++;
        printf("%d ", i);
    }   printf("\n Cnt : %d\n", cnt);
    // start  end  cnt  i  i<end-cnt  i%2==0  cnt++  출력  i++
    // --------------------------------------------------------
    start = 4, end=8;    cnt = 0;
    sum = 0;
    for(int i=start; i<end-cnt; i++){
    ...
    // start end cnt sum i i<end-cnt  i%2==0  Cnt++  sum+=i i++
    // --------------------------------------------------------
    // 대성아 vscode 글자 크기 조정방법 좀따 알려줄 것.
    // File->Preference->Setting->검색->wheel->위아래 두개
    // --------------------------------------------------------
    // break; -> 얘 만나는 순간 가장 가까운 반복문 와장창!
    start=1, end=10;
    for(int i=start; i<end; i++){
        if(i%2==0) continue; // start end i i<end i%2 i%7 출력 i++
        if(i%7==0) break;
        printf("%d ",i+1);
    }   printf("\n");
    printf("----------------\n");
