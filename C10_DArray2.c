...
int main(){
    ...
    // 4↑ 5↑ 6↑  -> 8 5 2  // 21 11 01
    // 7↑ 8↑ 9↑  -> 9 6 3  // 22 12 02
    for(int i=0; i<5; i++){
        for(int j=4-1; j>=0; j--){
            printf("%2d ", mat1[j][i]);
        }   EE();
    ...
    E();
    // ---------------------------------------
    // mat5 8x8에다가 64~1 거꾸로 저장 후
    // 모든요소를 출력하되, 두가지를 각 줄 뒤에 출력
    // 짝수의 합 EvenSum : 
    // 홀수의 합 OddSum : 
    // 마지막 줄은, 짝수들의 평균, 홀수들의 평균!
    // 8 7 6 5 4 3 2 1 EvenSum:20 OddSum:16
    // EvenAvg:?, OddAvg:? +  평균? 개수.
    int mat5[8][8] = {0};
    cnt = 8*8;
    for(int i=0;i<8;i++){
        for(int j=0; j<8; j++){
    ...
    // ㅠㅠㅠㅠ 매번마다 배열 계속 실행하는거 너무 힘들어 엉엉.....
