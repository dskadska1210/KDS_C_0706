...
// =========================================================
// < 미래의 나에게.... >
// 미안하다. 진도를 많이 못 나갔어. 너가 알아서 잘 나가주고
// 여기 파트 좀 안 푼 문제들 있어서 좀 더 해줘야행.
// =========================================================
...
// GetRandAry10은, size가 10인 int형 배열에 랜덤값이
...
// }
// void InitAry( int** dptr, int size ){
//     *dptr = (int*)malloc(size*sizeof(int));
//     for(int i=0; i<size; i++){
//         (*dptr)[i] = rand()*9+1 ; 
//     }
// }
...
// 방금 만든 함수들, 새로운 파일 만들어서
// 문제만 복사 후 , 새로 만들기.
// ---------------------------------------



// 포변과 배열길이, 변경할 배열길이를 입력받음
// 해당 배열의 길이를 변경할 배열 길이로 변경 후 다시
// 원본에 세팅해주는 함수 제작
...
int main(){
    ...
    size = 15;
    int* dy = (int*)malloc( size*sizeof(int) ); // int(4) * 15 = ary[size]
    for(int i=0; i<size; i++){
        *(dy+i) = i+1;
        printf("%d ", *(dy+i) );
    }
...
4. 변수의 주소는 "포인터 변수"가 저장할 수 있음
    배열은 첫 주소만 가지고 있으면, 누구나 사용 가능
    포인터 변수의 주소는 "이중 포인터 변수"가 저장가능
    이중포인터변수의 주소는 삼중 포인터 변수가저장가능
    삼중포인터변수의 주소는 사중 포인터 변수가저장가능
    사중포인터변수의 주소는 오중 포인터 변수가저장가능
    오중포인터변수의 주소는 육중 포인터 변수가저장가능
5. 배열을 사용하고 싶으면, 걔내 첫 집 주소가 필요
